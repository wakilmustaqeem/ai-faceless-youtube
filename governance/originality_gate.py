...
def _source_text(source: dict) -> str:
    return " ".join(str(source.get(k, "")).strip() for k in ("title", "summary", "excerpt", "evidence") if source.get(k)).strip()
def check(script: str, sources=None, threshold: float = DEFAULT_THRESHOLD) -> dict:
    ...
    if len(tokens) < MIN_TOKEN_COUNT:
        return {"passed": False, "reason": "script_too_short_for_originality_check", "token_count": len(tokens)}
    ...
    for source in usable:
        st = _tokens(_source_text(source))
        overlap = script_ngrams & Counter(_ngrams(st))
        longest = max((len(g) for g, count in overlap.items() if count), default=0)
        similarity = _cosine(Counter(tokens), Counter(st))
        matches.append({"title": source.get("title", ""), "url": source.get("url", ""), "cosine_similarity": round(similarity, 4), "shared_ngrams": sum(overlap.values()), "longest_shared_ngram": longest})
    if not matches:
        return {"passed": False, "reason": "no_comparable_source_evidence"}
    highest = max(matches, key=lambda x: x["cosine_similarity"])
    passed = highest["cosine_similarity"] < threshold and highest["longest_shared_ngram"] < 12
    return {"passed": passed, "reason": "originality_check_passed" if passed else "source_overlap_too_high", "threshold": threshold, "highest_similarity": highest["cosine_similarity"], "matches": matches}