...
def test_research_uses_real_source_verification(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda claims, sources: {
            "passed": True,
            "reason": "all_claims_source_verified",
            "claims": [
                {
                    "claim_id": "claim-1",
                    "status": "verified",
    ...
    result = researcher.run(
        ...
        [{"claim": "Fact", "source_url": "https://example.com", "source_title": "Example", "evidence": "verified evidence"}],
    )
...
def test_research_blocks_when_source_verification_fails(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda claims, sources: {
            "passed": False,
            "reason": "claim_source_verification_failed",
            "claims": [{"claim_id": "claim-1", "status": "blocked"}],
        },
    )
    result = researcher.run(
        "Topic",
        [{"url": "https://example.com", "title": "Example", "summary": "Summary"}],
        [{"claim": "Fact", "source_url": "https://example.com", "evidence": "missing"}],
    )
